...
public class PatchValidator
{
    ...
    private boolean save_testing_output;

    public PatchValidator(RepairInfo ri) {
	bug_id = ri.bugid;
	proj_testbuild_dpath = ri.projtestbuilddpath;
	dependjpath = ri.dependjpath;
	sharpfix_dpath = ri.sharpfixdpath;
	output_bugid_dpath = ri.outputbugiddpath;
	exec_dpath = ri.outputdpath; //NOTE: Time projects need resource files, we save the files under outputdpath.
	save_testing_output = true; //If not, I had the problem of non-termination (for compiling & testing). Have no idea why.
    }
	...
	String script_fpath = sharpfix_dpath+"/patchrunner.xml";
	//Compile the patch
