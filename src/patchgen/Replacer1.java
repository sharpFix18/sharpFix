...
class Replacer1
{
	...
	/*
	ASTNode core_node1 = getCoreASTNode(node1);
	ASTNode core_node2 = getCoreASTNode(node2);
	if (ASTNodeReduceChecker.isEqualOrReduced(core_node1, core_node2)) { //If node2 is a sub-component of node1, then only replace the nodes (and do no more). The modification is marked as replace_delete
	return rw_list;
	}

        if (isSimplified(core_node1, core_node2)) { //If node1 is a non-literal but node2 is, then simply replace the nodes (and do no more). The modification is marked as replace_simplify
	    return rw_list;
	}
	*/
	
	//Simply replace node1 with node2
	ASTRewrite rw0 = ASTRewrite.create(ast1);
	//rw0.replace(node1, node2, null);
	rw0 = replace(node1, node2, ast1, rw0);
	...
	//Replace components for certain types of nodes
	...
	*/
    ...
    private ASTRewrite replace(ASTNode node1, ASTNode node2, AST ast1, ASTRewrite rw0) {
	if ((node1 instanceof Statement) && (node2 instanceof Expression)) {
	    //Build a statement wrapper for node2
	    Expression node2_cp = (Expression) ASTNode.copySubtree(ast1, node2);
	    ExpressionStatement node2_stmt = ast1.newExpressionStatement(node2_cp);
	    rw0.replace(node1, (ASTNode) node2_stmt, null);
	    return rw0;
	}
	else if ((node1 instanceof Expression) && (node2 instanceof Statement)) {
	    if (node2 instanceof ExpressionStatement) {
		Expression node2_expr = ((ExpressionStatement) node2).getExpression();
		rw0.replace(node1, (ASTNode) node2_expr, null);
		return rw0;
	    }
	}
	rw0.replace(node1, node2, null);
