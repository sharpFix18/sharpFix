...
public class Modification implements Comparable<Modification>
{
	...
	if ("REPLACE_DELETE".equals(type2)) {
	    if (!("REPLACE_DELETE".equals(type))) {
		return -1;
	    }
	} else {
	    if ("REPLACE_DELETE".equals(type)) {
	...
	if ("REPLACE_SIMPLIFY".equals(type2)) {
	    if (!("REPLACE_SIMPLIFY".equals(type))) {
		return -1;
	    }
	} else {
	    if ("REPLACE_SIMPLIFY".equals(type)) {
	...
	//Either both are DETELE (SIMPLIFY)s or not DELETE (SIMPLIFY)s.
