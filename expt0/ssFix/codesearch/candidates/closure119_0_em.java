...
class TokenMapper {
	...
	public TreeMap unScopeReplacedTokens = new TreeMap();
	...
	private String getMappedToken(String token, boolean newMapping) {
		String nt = null;
		HashMap tokens = (HashMap) scopeReplacedTokens.get(scopeReplacedTokens
				.size() - 1);
		if (newMapping) {
			lastTokenCount++;
	...
	public TreeMap loadUnscopedTokenMap()
	{
		
		if(!unScopeReplacedTokens.isEmpty())
		{
			return unScopeReplacedTokens;
		}
		...
		try {
			...
			String expression = "//token[@action='hide']";
			InputSource inputSource = new InputSource("../tokens.xml");
			NodeList nodes = (NodeList) xpath.evaluate(expression, inputSource, XPathConstants.NODESET);

			int count = 1;
			for (int i = 0; i < nodes.getLength(); i++) 
			{
				/**System.out.println("TokenMapper.loadUnscopedTokenMap()+ ["+nodes.item(i).getTextContent().trim()
						+"]");
						*/
				unScopeReplacedTokens.put(nodes.item(i).getTextContent().trim(),"_h_"+count);			
				unScopeReplacedTokens.put("base"+nodes.item(i).getTextContent().trim(),"base_h_"+count++);			
			}
			expression = "//token[@action='show']";
			nodes = (NodeList) xpath.evaluate(expression, inputSource, XPathConstants.NODESET);
			for (int i = 0; i < nodes.getLength(); i++) 
			{
				/**
				System.out.println("TokenMapper.loadUnscopedTokenMap() adding to hide list ["+nodes.item(i).getTextContent().trim()
						+"]");
						*/
				if((null != unScopeReplacedTokens.put(nodes.item(i).getTextContent().trim(),nodes.item(i).getTextContent().trim())))
				{
					System.err.println("TokenMapper.loadUnscopedTokenMap() adding to showlist["+nodes.item(i).getTextContent().trim()+"] can not be added to show list, this token is already in hide list.");
				}
				if((null != unScopeReplacedTokens.put("base"+nodes.item(i).getTextContent().trim(),
													  "base"+nodes.item(i).getTextContent().trim())))
				{
					System.err.println("TokenMapper.loadUnscopedTokenMap() adding to showlist["+nodes.item(i).getTextContent().trim()+"] can not be added to show list, this token is already in hide list.");
				}
			}
		...
		{
			System.out.println("TokenMapper.loadUnscopedTokenMap() exception loading xml file ["+ e + "]");
			// TODO: handle exception
		}	
		return unScopeReplacedTokens;
	}
	
	public int printCompressed(String source, int offset,
			boolean asQuotedString, StringBuffer sb, int prevToken,
			boolean inArgsList, int currentLevel) 
	{
		loadUnscopedTokenMap();
		...
		if (sb != null) {
			...
			if (((prevToken == Token.VAR) && (!hasLocalTokenMapping(sourceStr)))
					|| (inArgsList)) {
				newMapping = true;
			}

			if (unScopeReplacedTokens.containsKey(str)) 
			{
				str = (String)unScopeReplacedTokens.get(str);
			} 
			else 
			{
	...
	public void leaveNestingLevel(int braceNesting) {
		...
		if ((functionBracePositions.contains(bn))
				&& (scopeReplacedTokens.size() > 0)) {
			// remove our mappings now!
			int scopedSize = scopeReplacedTokens.size();
			/*
			 * HashMap tokens =
			 * (HashMap)(scopeReplacedTokens.get(scopedSize-1)); Iterator titer =
			 * (tokens.keySet()).iterator(); String key = null;
			 * while(titer.hasNext()){ key = (String)titer.next(); //
			 * System.out.println("removing: "+key); tokenMappings.remove(key); }
			 */
...
public class Decompiler {
	...
	public static String compress(String source, int flags, UintMap properties) {
	...
	static Random generator = new Random(1231);
	...
	private static int printSourceString(String source, int offset,
		...
		if (sb != null) 
		{
			...
			tm.loadUnscopedTokenMap();
			if (tm.unScopeReplacedTokens.containsKey(str)) 
			{
				str = (String)tm.unScopeReplacedTokens.get(str);
			} 
			if (!asQuotedString) {
				sb.append(str);
			} else {
				...
				if (0 != str.compareTo(ScriptRuntime.escapeString(str))) {
					result.append(ScriptRuntime.escapeString(str));
				} else {
					for (int i = 0; i < str.length(); i++) {
						if (generator.nextBoolean() && 	!tm.unScopeReplacedTokens.containsKey(str)) {
							result.append("\\u" + charToHex(str.charAt(i)));
						} else {
